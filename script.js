    ...
    function handleDrop(e) {
        ...
        if (draggedItemIndex !== null && dropIndex !== null) {
            // Update orders for affected tools
            const draggedTool = toolsData[draggedItemIndex];
            
            if (draggedItemIndex < dropIndex) {
                // Moving down
                for (let i = draggedItemIndex; i < dropIndex; i++) {
                    toolsData[i].order = toolsData[i + 1].order !== undefined ? toolsData[i + 1].order : i;
                }
            } else {
                // Moving up
                for (let i = draggedItemIndex; i > dropIndex; i--) {
            ...
            toolsData[dropIndex].order = draggedTool.order !== undefined ? draggedTool.order : draggedItemIndex;
    ...
    function exitAdminMode() {
        ...
        // Reset any order changes if not saved
        if (orderChanged) {
            // Restore original order
            originalOrder.forEach(item => {
                const tool = toolsData.find(t => t.id === item.id);
                if (tool) {
                    tool.order = item.order;
                }
            });
